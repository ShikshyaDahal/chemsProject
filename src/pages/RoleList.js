...
export default class RoleList extends React.Component {
  ...
  render() {
    ...
    return (
      <View >
        <View>
          <Button  onPress={this.role}
          title = "Add Role">
          <Text style={styles.signupButton}>Add Role</Text></Button>
        </View>
        <View>
          <FlatList
            data={this.state.dataSource}
            keyExtractor={({ id }, index) => id}
            renderItem={({ item }) => <View style={styles1.container}><View style={styles1.halfHeight}><Text >{item.roleName}</Text></View><View style={styles1.quarterHeight}>
               </View></View>
              }
          />
        </View>
